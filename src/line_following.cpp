...
namespace jpmsc_stdr_reactive
{
	LineFollowing::LineFollowing(int argc, char **argv)
	{
		...
		{
			ROS_ERROR(
			"Usage : stdr_line following <robot_frame_id> <laser_frame_id>");
			exit(0);
		}
	...
	float LineFollowing::rTd(float radian)
	{
		return 	radian * (180 / 3.1415);
	}

	float LineFollowing::dTr(float degree)
	{
